...
```bash
npm run assets            # popup shots 01-03, letterboxed to 1280x800, + both promo tiles + store icon, then verified
npm run assets:details    # shot 04 (chrome://extensions Details); needs a display — on Linux: xvfb-run npm run assets:details
npm run assets:verify     # dimensions + 24-bit-no-alpha check on all seven assets
```

Everything is generated by Node + Chromium, so it runs on any OS; the `.ps1` equivalents are superseded and kept only for reference. Assets land in `store-assets/`, which is gitignored — they are rebuilt on demand rather than committed.
...
| `scripts/letterbox-screenshots.js` | Centres the popup shots on the store's 1280×800 canvas.                       |
| `scripts/make-promo.js`         | Draws the 440×280 and 1400×560 promo tiles.                                       |
| `scripts/make-store-icon.js`    | Flattens the 128×128 listing icon to 24-bit RGB.                                  |
| `scripts/verify-store-assets.js`| Checks every asset's dimensions and that no alpha channel remains.                |
| `scripts/lib/png.js`            | Tiny PNG decode/encode used to hit the store's 24-bit-no-alpha requirement.        |
