...
| Small promo tile       | 440×280 PNG (**required by store**) | `store-assets/promo-440x280.png`      | **regenerate for v1.2.0** — `scripts/make-promo.ps1` now writes tagline "Fix Zoom cookies in one click." + subline "One button. Zoom cookies only. Reload."; uses existing 1132 icon + dark/amber scheme + wordmark `1132 FIXER` + explicit "Independent project. Not affiliated with Zoom." disclaimer. No Zoom logo. |
...
| - | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ----------------------- | -------- |
| 1 | `store-assets/01-zoom-detected.png`                      | Popup open on `https://zoom.us`.                                                                              | State pill reads `READY · zoom.us`, the single **FIX ZOOM** button is rendered, footer chip reads `COOKIES ONLY`.           | **yes**     | regenerated by `scripts/capture-screenshots.js` against the v1.2.0 popup            |
| 2 | `store-assets/02-fix-complete.png`                       | Popup immediately after a successful **FIX ZOOM** click.                                                       | State pill reads `CLEARED` and the result line reports the outcome (a cookie count on a real session; "No Zoom cookies were left to remove." against the empty mocked jar). | **yes**     | regenerated; result line preserved verbatim                                        |
| 3 | `store-assets/03-non-zoom-safe.png`                      | Popup on a clearly non-Zoom site such as `https://example.com`.                                               | State pill reads `NOT ZOOM` in neutral grey, **no button at all**, one line telling the user to open a Zoom tab.            | **yes**     | regenerated for the v1.2.0 non-Zoom state                                          |
| 4 | `store-assets/04-extension-details-permissions.png`      | `chrome://extensions` → **Details** page for 1132 Fixer.                                                      | Shows real `1132 Fixer for Chrome` Details page (name, version `1.2.0`, ID, description, Site access scoped to Zoom hosts, no browsing-data permission). | recommended | **re-capture required for v1.2.0** — the v1.1.0 shot still lists the old permissions |
...
| Real Chromium load-unpacked via Playwright `launchPersistentContext --load-extension` | **stale** — `scripts/capture-extension-details.js` passed on v1.1.0 (extension card `On`, ID, Site access scoped to Zoom hosts). Re-run it on v1.2.0 so screenshot #04 shows the reduced permission list. |
...
Not covered here:             live signed-in zoom.us walk, and screenshot #04 re-capture on v1.2.0
```
